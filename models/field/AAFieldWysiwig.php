...
class AAFieldWysiwig extends AAFieldText
{
	...
	public function formInput(&$controller, $tagOptions=array())
	{
		...
		$tinyMceOpts = array(
			...
			'theme_advanced_buttons2' => 'cut,copy,paste,pastetext,pasteword,|,search,replace,|,bullist,numlist,|,outdent,indent,blockquote,|,undo,redo,|,link,unlink,anchor,iupload,image,cleanup,help,code,|,insertdate,inserttime,preview,|,forecolor,backcolor',
		...
		$nameID = 'aa'.substr(md5($inputID), 0, 8).'_';
		?>
<script type="text/javascript">
var <?=$nameID?>options = <?=CJSON::encode($tinyMceOpts)?>;
var <?=$nameID?>editor = null;
<?=$nameID?>options.setup = function(ed) {
	ed.addButton('iupload', {
		title : '<?=Yii::t('AutoAdmin.form', 'Upload an image')?>',
		image : '<?=AutoAdmin::$assetPath?>/images/img-upload.png',
		onclick : function() {
			<?=$nameID?>editor = ed;
			aaImgUploadWindow($('[id="'+ this.id +'"]'), '<?=$nameID?>ImgUploadCallback');
		}
	});
};
function <?=$nameID?>ImgUploadCallback(fieldName, code)
{
	<?=$nameID?>editor.focus();
	<?=$nameID?>editor.selection.setContent(code);
}
</script>
		<?
		Yii::app()->clientScript->registerScript("js_{$inputID}", "\$('[id=\"{$inputID}\"]').tinymce({$nameID}options)");
		return ob_get_clean();
	}
